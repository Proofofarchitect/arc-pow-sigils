...
const RC = [
  ...
  [0x00008089, 0x80000000], [0x80000003, 0x80000000],
  [0x80008002, 0x80000000], [0x00000080, 0x80000000],
...
function runBatch() {
  ...
  for (let count = 0; count < batchSize; count++) {
    ...
    S[20] = readLE32(nonce, 4);
    S[22] = readLE32(nonce, 8);
    S[24] = readLE32(nonce, 12);
    S[26] = readLE32(nonce, 16);
    S[28] = readLE32(nonce, 20);
    S[30] = readLE32(nonce, 24);
    S[32] = readLE32(nonce, 28);
  ...
  if (elapsed >= 250) {
    ...
    const hps = config.attempts / (totalElapsed / 1000);
